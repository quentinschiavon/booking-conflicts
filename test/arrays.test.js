...
// import
const { sortedIndex,sortedItemIndex, insert,insertItem } = require('../utils/arrays')
...
describe('# Array', function() {
  ...
  describe('## sortedItemIndex()', function() {
    ...
    it('should return the right index', function() {
        let res = sortedItemIndex([[1,1],[2,-1],[3,-1]],[0,0])
        expect(res).to.be.equal(0);
        res = sortedItemIndex([[1,1],[2,-1],[3,-1]],[1,0])
        expect(res).to.be.equal(0);
        res = sortedItemIndex([[1,1],[2,-1],[3,-1]],[3,0])
        expect(res).to.be.equal(2);
    });
  });
  ...
  describe('## insertItem()', function() {
    ...
    it('should return a sorted array', function() {
      ...
      res.should.be.an('array');
      res.length.should.be.equal(3);
      expect(res).to.deep.equal([[0,1],[1,-1],[2,4]]);
    ...
    it('should not insert an item if not a two length array', function() {
      ...
      expect(res).to.deep.equal([[0,1],[2,4]]);
    });
  });
});
